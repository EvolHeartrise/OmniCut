...
import { loadAllChannelSettings, loadWatchlist, loadWordTimestamps } from './persistence.js';
...
export function createMcpServer(): McpServer {
	...
	mcpServer.tool(
		...
		'List clip regions, optionally filtered by stream ID, channel name, and/or a datetime range. Times are ISO 8601 datetime strings (e.g. "2026-02-20T14:30:00Z").',
		{
			...
			after: z
				.string()
				.optional()
				.describe('Only include clips that end after this datetime (ISO 8601, e.g. "2026-02-20T14:30:00Z")'),
			before: z
				.string()
				.optional()
				.describe('Only include clips that start before this datetime (ISO 8601, e.g. "2026-02-21T02:00:00Z")')
		},
	...
	mcpServer.tool(
		...
		'Search chat messages in a time range across one or multiple streams. Times are in stream-local seconds (seconds since capture started). Supports optional text filtering (substring or regex) and pagination.',
		{
			...
			query: z
				...
				.optional()
				.describe(
					'Text filter — only return messages matching this string. Substring match by default, or regex if useRegex is true.'
				),
			useRegex: z
				.boolean()
				.optional()
				.default(false)
				.describe('Treat query as a regular expression (case-insensitive)'),
	...
	mcpServer.tool(
		...
		'Search transcription segments in a time range across one or multiple streams. Times are in stream-local seconds. Supports optional text filtering (substring or regex) and pagination.',
		{
			...
			query: z
				...
				.optional()
				.describe(
					'Text filter — only return segments matching this string. Substring match by default, or regex if useRegex is true.'
				),
			useRegex: z
				.boolean()
				.optional()
	...
	mcpServer.tool(
		...
		'Look up a channel to check if it is live, get its title, game, viewer count, and VOD availability.',
		{
			...
			platform: z.enum(['twitch', 'douyu']).optional().default('twitch').describe('Platform (default: twitch)')
		},
	...
	mcpServer.tool('get_channel_settings', 'Get per-channel transcription language settings.', {}, async () => {
	...
	mcpServer.tool(
		...
		'Query a stream at a specific timestamp. Returns stream details and a window of all chat messages and transcription segments centered on the given timestamp.',
		{
			...
			windowSeconds: z
				...
				.optional()
				.default(30)
				.describe('Total window size in seconds (default 30, centered on timestamp)')
		},
	...
	mcpServer.tool(
		...
		'Create a clip region on a stream. Defines a time range that can later be exported as video. Times default to master time (epoch seconds) but can be stream-local seconds if timeFormat is set to "local".',
		{
			...
			timeFormat: z
				...
				.default('master')
				.describe(
					'Time format: "master" for epoch seconds (default), "local" for stream-local seconds since capture start'
				),
	...
	mcpServer.tool(
		...
		'Update an existing clip region. Can change start/end times, title, and/or notes. Only provided fields are updated; omitted fields keep their current values.',
		{
			...
			startTime: z
				.number()
				.optional()
				.describe('New start time (epoch seconds by default, or stream-local if timeFormat is "local")'),
			endTime: z
				.number()
				.optional()
				.describe('New end time (epoch seconds by default, or stream-local if timeFormat is "local")'),
			timeFormat: z
				.enum(['master', 'local'])
				.optional()
				.default('master')
	...
	mcpServer.tool(
		...
		'Export a video from one or more clips, stitched in the specified order. Returns immediately with an export ID — the encode runs in the background. Use list_exports to check status.',
		{
			...
			chronological: z
				...
				.optional()
				.default(false)
				.describe('If true, automatically sort clips by start time instead of using the provided order')
		},
